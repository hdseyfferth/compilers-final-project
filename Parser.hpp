...
private:
  ...
  [[nodiscard]] ASTNode MakeVarNode(Token var_token) const {
    ...
    Type type = symbols.GetType(var_id);
    return ASTNode_Var(var_token, var_id, type);
  }

  void Parse_GlobalVar() {
    Token token_type = lexer.Use(Lexer::ID_TYPE);
    Token token_id = lexer.Use(Lexer::ID_ID);

    // Add global var to symbol table
    symbols.AddGlobalVar(token_id, token_type);

    // Check for a global initialization
    if (lexer.Peek() == '=') {
      lexer.Use('=');  // Consume '='
      ASTNode init_expr = Parse_Expression();
    
    // Store the initialization expression
      symbols.AddGlobalInit(token_id.lexeme, std::move(init_expr));
    }
    // Consume semicolon
    lexer.Use(';');
  }
  ...
  [[nodiscard]] ASTNode Parse_Statement() {
    ...
    switch (token_id) {
    case Lexer::ID_BREAK:    out_node = Parse_BREAK();    break;
    case Lexer::ID_CONTINUE: out_node = Parse_CONTINUE(); break;
    case Lexer::ID_TYPE:     out_node = Parse_Declare();  break;
    case Lexer::ID_RETURN:   out_node = Parse_RETURN();   break;

    // Statements that DON'T need a semi-colon at the end:
    case Lexer::ID_IF:       return Parse_IF();
    case Lexer::ID_WHILE:    return Parse_WHILE();
    case '{':                return Parse_Block();

    case ';': break; // Empty line -- nothing to process.
    default: out_node = Parse_Expression();
    }
  ...
  [[nodiscard]] ASTNode Parse_Block() {
    ...
    while (lexer.Peek() != '}') {
      ...
      if (line.GetNodeType() != ASTType::UNKNOWN) block.AddChild( std::move(line) );
    }
  ...
  [[nodiscard]] ASTNode Parse_Term() {
    ...
    // Check if this is an ID.
      ...
      // Check for function calls before variables
      if (symbols.HasFunSymbol(token.lexeme)) {
        // Get unique id for function
        size_t fun_id = symbols.GetFunID(token.lexeme);
        if (lexer.Peek() == '(') {
          lexer.Use('(');
          ASTNode fun_node = ASTNode_FunCall(token, fun_id);
          while (lexer.Peek() != ')') {
            // Parse each argument as expression
            fun_node.AddChild(Parse_Expression());
            if (lexer.Peek() == ',') {
              lexer.Use(',');
            }
          }
          lexer.Use(')', "Missing parenthesis");
          out_node = std::move(fun_node);
        }
      }
    ...
    // == Check if this is a literal ==
      ...
      out_node = ASTNode_LitString(token);
    }
    else if (token == Lexer::ID_LIT_CHAR) {
      // Extract char
      char lit_char = token.lexeme[1];
      // Convert from char to ascii value int
      int ascii = static_cast<int>(lit_char);
      Token out_token = token;
      // Convert to int literal for node creation
      out_token.lexeme = std::to_string(ascii);
      
      out_node = ASTNode_LitInt(out_token);
    }
    else Error(token, "Unexpected token '", token.lexeme, "'");;

    if (lexer.Peek() == '[') {
      out_node = Parse_Index(std::move(out_node));
    }
    return out_node;
  }

  [[nodiscard]] ASTNode Parse_Index(ASTNode str_expr) {
      Token index_token = lexer.Use('[');
      ASTNode index_expr = Parse_Expression();
      lexer.Use(']');

      str_expr = ASTNode_Index(index_token, std::move(str_expr), std::move(index_expr));
      return str_expr;
  }

  [[nodiscard]] ASTNode Parse_Expression(size_t prec_level=1000, ASTNode lhs=ASTNode{}) {
    prec_level = std::min(prec_level, max_prec_level);

    if (prec_level == 0) {
      // If we aren't extending an existing LHS expression, get a term.
      if (lhs.GetNodeType() == ASTType::UNKNOWN) lhs = Parse_Term();
      return lhs;
    }
    ...
    if (it != op_map.end() && it->second.prec == prec_level) {
      ...
      size_t right_prec = it->second.recurse_right ? prec_level : prec_level - 1;
      ...
      if (it->second.recurse_left) lhs = Parse_Expression(prec_level, std::move(lhs));
    }
    return lhs;
  }

  [[nodiscard]] ASTNode Parse_BREAK() {
    Token break_token = lexer.Use(Lexer::ID_BREAK);
    return ASTNode_Break(break_token);
  }

  [[nodiscard]] ASTNode Parse_CONTINUE() {
    Token cont_token = lexer.Use(Lexer::ID_CONTINUE);
    return ASTNode_Continue(cont_token);
  }

  [[nodiscard]] ASTNode Parse_RETURN() {
  ...
  [[nodiscard]] ASTNode Parse_IF() {
    Token if_token = lexer.Use(Lexer::ID_IF);
    lexer.Use('(');
    ASTNode condition = Parse_Expression();
    lexer.Use(')', "If condition must end with ')', but found \"", lexer.Peek().lexeme, "\".");
  ...
  void Parse_Function() {
    ...
    while (lexer.Peek() != ')') {
      ...
      if (lexer.Peek() != ')') {
        ...
        lexer.Use(',', "Function parameters must be separated by commans (',')");
      }
      symbols.AddSymbol(param, type_id, true);
    }
    lexer.Use(')');
    lexer.Use(':');
...
public:
  Parser(Lexer & lexer, SymbolTable & symbols) : lexer(lexer), symbols(symbols) {
    ...
    op_map["<"] = op_map[">"] = op_map["<="] = op_map[">="] = op_map["=="] = op_map["!="] = cur_info;
  ...
  void Parse() {
    ...
    while (lexer.Any()) {
      if (lexer.Peek() == Lexer::ID_TYPE) {
        Parse_GlobalVar();
      }
      else if (lexer.Peek() == Lexer::ID_FUNCTION) {
        Parse_Function();
      }
      else {
